...
VALID_DOMAINS = {
    "lootlabs": ["loot-link.com", "lootdest.org"],
    "linkvertise": ["link-hub.net", "direct-link.net", "link-target.net","linkvertise.com"]
}

# Discord OAuth Configuration
DISCORD_CLIENT_ID = "1392575706705428602"
DISCORD_CLIENT_SECRET = "aycLhuw5B3tCYmD9Sz57uT2sfbbIj_Qr"
...
# Discord Role IDs (from your bot)
DISCORD_ROLE_IDS = {
    "free": [1391839228458958889, 1418391115714924665],
    "boost": [1404627044960309329, 1425870518451568700, 1418391115714924665],
    "premium": [1391833734461919483, 1428920662852108441, 1418391115714924665],
    "pro": [1418391115714924665],
    "no_cooldown": [1428453528464265277, 1418391115714924665],
...
class DiscordAuth:
    ...
    def get_user_data(self, access_token):
        ...
        # Get user info
        user_response = requests.get(f'{self.api_base}/users/@me', headers=headers)
        if user_response.status_code != 200:
            return None
            
        user_data = user_response.json()
        
        # Get user's guild member data using bot token
        bot_headers = {
            'Authorization': f'Bot {self.bot_token}'
        }
        
        member_response = requests.get(
            f'{self.api_base}/guilds/{self.guild_id}/members/{user_data["id"]}', 
            headers=bot_headers
        )
    ...
    def get_user_tier(self, user_roles):
        """Determine user's highest tier based on roles"""
        user_role_ids = set(user_roles)
        
        # Check in order: pro -> premium -> boost -> free
    ...
    def has_no_cooldown(self, user_roles):
        user_role_ids = set(user_roles)
        return any(role_id in user_role_ids for role_id in self.role_ids["no_cooldown"])

    def is_admin(self, user_roles):
        user_role_ids = set(user_roles)
        return any(role_id in user_role_ids for role_id in self.role_ids["admin"])
...
def load_codes():
    ...
    for i, code in enumerate(codes):
        if isinstance(code, str):
            codes[i] = {"code": code, "used": False, "claimed_by": None, "claimed_at": None, "claimed_ip": None}
            updated = True
    if updated:
        save_codes(codes)
    return codes
...
def is_valid_referrer(referrer, level):
    ...
    for sponsor_type in ["lootlabs", "linkvertise"]:
        ...
        for domain in VALID_DOMAINS[sponsor_type]:
...
def can_claim_code():
    if "claimed_ips" not in session: session["claimed_ips"] = []
    ip_history = session["claimed_ips"]
    myip = ip()
    return myip not in ip_history

def remember_claimed_code():
    if "claimed_ips" not in session: session["claimed_ips"] = []
    myip = ip()
    if myip not in session["claimed_ips"]: session["claimed_ips"].append(myip)

# Clear user keys and reset session
def clear_user_keys():
    # Clear session data
    session.pop('user_code', None)
    session.pop('claimed_ips', None)
    session.pop('task_started', None)
    session.pop('task_started_time', None)
    session['level'] = 1
    
    # Remove unused keys for this IP from database
    myip = ip()
    codes_db = load_codes()
    updated_codes = []
    
    for code_obj in codes_db:
        # Keep used codes and codes from other IPs
        if code_obj.get("used") or code_obj.get("claimed_ip") != myip:
            updated_codes.append(code_obj)
    
    if len(updated_codes) != len(codes_db):
        save_codes(updated_codes)
        return True
    return False

# Get keys for current IP
def get_user_keys():
    myip = ip()
    codes_db = load_codes()
    user_keys = [code for code in codes_db if code.get("claimed_ip") == myip]
...
def get_available_services(user_tier):
    """Get available services based on user tier"""
    available = []
    for service, tiers in SERVICE_FILES.items():
        # Check if user's tier has access to this service
        if user_tier in tiers:
            # Also check if the stock file exists and has content
            filename = tiers[user_tier]
            if os.path.exists(filename):
                try:
                    with open(filename, 'r', encoding='utf-8') as f:
                        accounts = [line.strip() for line in f if line.strip()]
                    if accounts:  # Only add service if there are accounts available
                        available.append(service)
                except:
                    pass
            else:
                # File doesn't exist, but user has access - still show the service
                available.append(service)
    
    # If no services found, show some default services for debugging
    if not available and user_tier != "none":
        print(f"DEBUG: No services found for tier {user_tier}, showing defaults")
        available = ["roblox", "minecraft", "discord"]
...
def discord_callback():
    ...
    session['user_tier'] = discord_auth.get_user_tier(user_data['roles'])
    session['has_no_cooldown'] = discord_auth.has_no_cooldown(user_data['roles'])
    session['is_admin'] = discord_auth.is_admin(user_data['roles'])
...
def progress():
    ...
    if level > MAX_LEVEL: return redirect(url_for('code_page'))
    nav = step_nav(level)
    ...
    progress_hash = generate_progress_hash(level, session_token)
    claim_link = url_for(f'claim{level}')  # Use step-specific claim endpoint
    ...
    if claim_enabled:
        ...
        <form action="{claim_link}" method="get" style="margin-top:1.2em;">
            <button class="progress-btn" type="submit">Claim Step</button>
        </form>
        """
    sponsor_buttons = ""
    if lootlabs_url:
        sponsor_buttons += f'<a href="{url_for("start_task", lv=level, token=session_token, sponsor="lootlabs")}" target="_blank"><button class="progress-btn">LootLabs</button></a>'
    if linkvertise_url:
        sponsor_buttons += f'<a href="{url_for("start_task", lv=level, token=session_token, sponsor="linkvertise")}" target="_blank" style="margin-left:0.7em;"><button class="progress-btn">Linkvertise</button></a>'
...
def start_task():
    try: level = int(request.args.get('lv', 1))
    except: level = 1
...
def claim1():
    ...
    if not is_valid_referrer(ref, 1):
        ...
        session.pop('task_started', None)
        session.pop('task_started_time', None)
        return render_template('claim_error.html', is_mobile=is_mobile_user())
    cur = session.get('level', 1)
    task_started = session.get('task_started')
    if 1 != cur or task_started != 1:
    ...
    session.pop('task_started', None)
    session.pop('task_started_time', None)
    if cur + 1 > MAX_LEVEL: return redirect(url_for('code_page'))
...
@app.route("/code")
def code_page():
    if session.get('level', 1) <= MAX_LEVEL: return redirect(url_for('progress'))
...
def key_management():
    current_ip = ip()
    return render_template('key_management.html', current_ip=current_ip, is_mobile=is_mobile_user())

# New route to show keys for current IP
@app.route("/show_keys")
def show_keys():
    current_ip = ip()
    user_keys = get_user_keys()
    return render_template('show_keys.html', keys=user_keys, current_ip=current_ip, is_mobile=is_mobile_user())
        ...
        if search_filter == "key":
            ...
            filtered_codes = [
                ...
                if (search_term in code.get("code", "").upper() or
                    ...
                    (code.get("claimed_ip") and search_term in code.get("claimed_ip", "")))
            ]
...
def admin_export_free_keys():
    ...
    writer.writerow(['Key', 'Status', 'Claimed By', 'Claimed At', 'IP Address'])
